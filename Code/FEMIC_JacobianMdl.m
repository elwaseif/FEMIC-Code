function [G,J] = FEMIC_JacobianMdl(freq,rspa,params,d,N,M,nlayers,q)
...
% Code delivered to the USGS in 2008
%
%%
% INPUTS:
%   freq = frequencies
...
%   J = Jacobian matrix representing the 1st derivative the forward model
%   with respect to each model parameter. (computed numerically via
%   centered finite difference)
%
% EXAMPLE USAGE:
% [G,J] = jacobianMdl(freq,rspa,params,d,N,M,nlayers,q)
% USEAGE EXAMPLE:
...
%time1 = toc; fprintf('Gauss 2 time = %f\n',time1);tic;
    ...
    for j=1:M,
        psigma(j) = sigma(j)+1e-5;
        if j>nlayers,
            plyr(j-nlayers) = lyr(j-nlayers) + 1e-5;
            denom = 2.0e-5;
        else
            denom = 2.0e-5;
        end
        if q~=2,
            [tvaljhi0] = gaussinterp(r2,psigma,0,w,plyr,N,nlayers,aint1,aint2,gnpts);
        end
%time1 = toc; fprintf('Gauss T0 1 time = %f\n',time1);
        if q~=1,
            [tvaljhi2] = gaussinterp(r2,psigma,2,w,plyr,N,nlayers,aint1,aint2,gnpts);
        end
%time1 = toc; fprintf('Gauss T2 1 time = %f\n',time1);
        if q~=2,
            jhi2=(4.0/(muo*w*r2^2.0))*(-tvaljhi0(2)*(r2^3.0));
        end
        if q~=1,
            jhi4=(4.0/(muo*w*r2^2.0))*(-tvaljhi2(2)*(r2^2.0));
        end
        psigma(j) = sigma(j)-1.0e-5;
        if j>nlayers,
            plyr(j-nlayers) = lyr(j-nlayers) - 1e-5;
            denom = 2.0e-5;
        else
        ...
        if q~=2,
            [tvaljlo0] = gaussinterp(r2,psigma,0,w,plyr,N,nlayers,aint1,aint2,gnpts);
            jlo2=(4.0/(muo*w*r2^2.0))*(-tvaljlo0(2)*(r2^3.0));
        end
%time1 = toc; fprintf('Gauss T0 2 time = %f\n',time1);
        if q~=1,
            [tvaljlo2] = gaussinterp(r2,psigma,2,w,plyr,N,nlayers,aint1,aint2,gnpts);
            jlo4=(4.0/(muo*w*r2^2.0))*(-tvaljlo2(2)*(r2^2.0));
        end
%time1 = toc; fprintf('Gauss T2 2 time = %f\n',time1);
        
        % Compute finite difference jacobian
        if q~=2,
            J(k,j)=(jhi2-jlo2)/(denom);
        end
        if q~=1,
            J(k+N,j)=(jhi4-jlo4)/(denom);
        end
        if q==2,
            J(k,j)=(jhi4-jlo4)/(denom);
        end        
        %if j>nlayers; keyboard; end
        psigma(:)=sigma(:);
        plyr(:) = lyr(:);
    end  % end loop over model parameters
%time1 = toc; fprintf('J computation time = %f\n',time1);tic;
...
fo=length(G);  
if q==2
       G(length(freq)+1:fo)=0;
       J(length(freq)+1:fo,1)=0;
end
...
nu2 = 0.*[1:30]'+1.*[1:30]'*1i;
v = 0.*[1:30]'+1.*[1:30]'*1i;
rtop = 0.*[1:30]'+1.*[1:30]'*1i;
rbot = 0.*[1:30]'+1.*[1:30]'*1i;
ro = 0.*[1:30]'+1.*[1:30]'*1i;
vik = 0.*[1:30]'+1.*[1:30]'*1i;
va = 0.*[1:30]'+1.*[1:30]'*1i;
vb = 0.*[1:30]'+1.*[1:30]'*1i;
vc = 0.*[1:30]'+1.*[1:30]'*1i;
M=M+1;
...
for ij=M:-1:2,
    va(ij) = (v(ij-1)-v(ij));
    vb(ij) = (v(ij-1)+v(ij));
    vc(ij) = ro(ij)*exp(-2.0*nd(ij)*v(ij));
