...
function [p_final, muh_final, rms_error2, G]=FEMIC_tost22(MM,S,el,pobs,sigma,muv,err_tol,max_iter,q,sx,sz,wta)
...
while(norm(gc)/normg0 > err_tol & itc < max_iter & norm(gc)>1e-20)
  ...
  dx=ones(P,1);dz=MM.thk;%ones(length(MM.thk),1);dy=dx;
  nx=length(dx);nz=length(dz);V = spdiags((wta(:)), 0, nx*nz, nx*nz); 
  [Gx, Gz]=grad2d(dx,dz);als=7;%V=0;
   Gs = [sx*Gx;sz*Gz];
