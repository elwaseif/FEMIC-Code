function [rawGem, calGem, filtGem]=new_calibration(dcale,dobsy,f,r,q,vall)
...
parfor ii=1:length(f)
	depth(ii)=503/sqrt(f(ii)*m);
	d2(:,ii)=abs(dcale(:,3)-depth(ii));
	ff=min(d2(:,ii));
	fdepth(:,ii)=find(d2(:,ii)==ff);
end
...
dcall=dcale(:,4);dcali=reshape(dcall,length(f),length(dd)/length(f));%dlmwrite('dcali.dat',dcali);
warning off

dobss=dobsy(:,8);xx2=dobsy(:,1);yy2=dobsy(:,2);
rawGem=dobss;
coor1=[xx,yy];
coor2=[xx2,yy2];
[m n]=size(coor1);
...
dcali=dcali(:);dcali2=reshape(dcali,length(f),length(dcali)/length(f));
[dcal]=FEMIC_forward2D(dcali2,depth,vall);dcal=dcal';
fprintf('okforward')
dobsa=dobss(boo);
dobs=reshape(dobsa,length(f),length(dobsa)/length(f));dobs=dobs';
...
for n = 1:10   
    ...
    dprd = (dcal + repmat(B,ns,1))*diag(G);
    dcor = dobs*diag(1./G) - repmat(B,ns,1);calGem=dcor;
...
dcor=dcor';dcor2=dcor(:);
dobss(boo)=dcor2;%dlmwrite('dob.dat',dobss);
