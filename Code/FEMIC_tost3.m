...
function [p_final, muh_final, rms_error2, G]=FEMIC_tost3(MM,S,el,pobs,sigma,muv,err_tol,max_iter,q,sx,sy,sz,wta,pmin,pmax)
...
P=size(pobs,2);               % number of data (freq x 2)P=size(pobs,2);
PP=size(pobs,3);
PPP=size(pobs,1);
...
%for i=1:max_iter,       % loop over iterations (A1)
  ...
  dx=1*ones(P,1);dz=ones(PPP,1);dy=ones(PP,1);%dlmwrite('wt.dat',wta);
  nx=length(dx);nz=length(dz);ny=length(dy);V = spdiags((wta(:)), 0, nx*ny*nz, nx*ny*nz); 
...
pin=pin(:);
   ...
   while 1,
     ...
     for ii=1:P*PP,%% Loop over all station positions (B2)
      ...
      fm = 0.5*muv*( (xt(:)-mref)'*muv*(xt(:)-mref))+(X'*X);       
      ft = fd+fm;
...
p_final=xt; p_final=reshape(p_final,PPP,P*PP);
muh_final=muv;
